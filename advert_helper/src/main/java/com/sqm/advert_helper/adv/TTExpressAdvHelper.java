...
public class TTExpressAdvHelper implements LifecycleObserver {
    ...
    @StringDef({TYPE_MAIN, TYPE_QUIT, TYPE_ACTIVE, TYPE_OTHER,})
    @Retention(RetentionPolicy.SOURCE)
    public @interface AdvTypeConfig {
    }
    private static String getString(@NonNull String advType) {
        String location;
        switch (advType) {
            case TYPE_MAIN:
                location = "mainad";
...
//        this(activity, "quit",-1,-1);
    ...
    public TTExpressAdvHelper(ComponentActivity activity, String advType) {
        this(activity, getString(advType), -1, -1);
    }
