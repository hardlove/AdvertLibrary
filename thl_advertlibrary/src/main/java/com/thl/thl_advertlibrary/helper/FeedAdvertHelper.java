 ...
 * @time 2020/1/7 15
 */
public class FeedAdvertHelper {

    public interface OnFeedAdvertListener {
     ...
     * @param model
     */
    public FeedAdvertHelper(AppCompatActivity activity, AdvertModel model) {
        this.activity = activity;
        this.model = model;
    }

    public FeedAdvertHelper setListener(OnFeedAdvertListener listener) {
    ...
    public void initAdvert(RelativeLayout rl_content) {
        ...
        rl_content.post(new Runnable() {
            ...
            public void run() {
                ...
                float screenWidth = rl_content.getWidth() / Fhad_DeviceUtil.Density(activity);  // 屏幕宽度(dp)
                if (screenWidth==0){
                    screenWidth = rl_content.getMeasuredWidth() / Fhad_DeviceUtil.Density(activity);
                }
                int height = (int) (screenWidth * model.getHeight() / model.getWidth());
                AdSlot adSlot = new AdSlot.Builder()
                        .setCodeId(model.getAdvert_param_1())
                        .setSupportDeepLink(true)
                        .setExpressViewAcceptedSize(screenWidth, height) //期望模板广告view的size,单位dp
    ...
    private void bindAdListener(RelativeLayout rl_content,TTNativeExpressAd ad, OnFeedAdvertListener listener) {
